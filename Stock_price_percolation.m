...
while pricecheck ~= 1
    ...
    while day <= days
        ...
        %site if it is 1, it will then create a rand number if the number
        %is below the influence rate, the neighbour will take an action,
        %sell or buy depending on the influence rate. If PU is used as
        %infleunce rate, the action will be to buy and the opposite for the
        %other case.This process repeats for each direction and so on.
